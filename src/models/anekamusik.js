...
module.exports = {
    ...
    getProducts: (data) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            db.query(`SELECT p.id, c.category_name, b.branch_name, p.name, p.quantity, p.description, p.price, p.url FROM products AS p INNER JOIN category AS c ON (p.id_category = c.id) INNER JOIN branch AS b ON (p.id_branch = b.id) LIMIT ${page},${data.limit}`, (err, response) => {
                if (!err) {
                    resolve(response)
                } else {
                    reject(err)
